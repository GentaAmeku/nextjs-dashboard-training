...
## 11-5. ハンズオン：`lib/result.ts` と `lib/errors.ts` を実装する
...
## 11-7. ハンズオン：`taskRepository` を実装する
...
次の第 12 章では、バリデーションとビジネスロジックを担う **Service 層** を実装します。
