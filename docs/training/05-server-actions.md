...
## 5-3. このプロジェクトの Server Actions を実装する
...
> [!TIP]
> **この章の JSX（フォームのマークアップ）はリポジトリからコピーして進めて OK です。**
> 本章のメインは「Server Action の定義方法」と「`useActionState` でのフォームとの接続」を理解することです。
...
次の第 06 章では **Better Auth と Google OAuth** を設定し、ログイン・ログアウトと認証ガードを完成させます。
