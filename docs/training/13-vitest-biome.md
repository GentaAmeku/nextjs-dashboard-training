...
## 13-5. Repository テストのパターン確認

第 11 章で書いた Repository テストのパターンを復習します。

```typescript
// lib/db/repositories/__tests__/task-repository.test.ts（第 11 章参照）
// 重要なポイント：
...
## 13-6. Service テストのパターン確認

第 12 章で書いた Service テストのパターンを復習します。

```typescript
// lib/db/services/__tests__/task-service.test.ts（第 12 章参照）
// 重要なポイント：
...
次の第 14 章では **React Compiler と Turbopack** について、なぜ `useMemo` を書かないかを読み物形式で学びます。
