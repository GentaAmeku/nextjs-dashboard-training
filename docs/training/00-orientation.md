...
完成形のコードは回答リポジトリ（`answer` リモート＝ dashboard-playground-nextjs）にすべて入っています。まずは完成版を動かして「何を作るのか」を体感してから、各章で順番に実装していきましょう。
...
# 2. 環境変数の設定（第 06 章で詳しく説明します。今は仮の値でも OK）
...
> 第 06 章で Google Cloud Console の設定を一緒に行います。
